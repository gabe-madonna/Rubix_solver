...
class Cube(object):
    ...
    def turn(self, face, d = 1):
        ...
        d: 1 or -1 multiplier (1 is clockwise, -1 i counter clockwise)
        '''
        if d == 1:
    ...
    def prune_directions(self):
        ...
        '''
        dirs_string = ' '.join(self.directions)
        
        for side in SIDES: #replace triple turns with single turn other direction
            repeat = 3 * (side + '+ ')
            dirs_string.replace(repeat, side + '- ')
            repeat = 3 * (side + '- ')
            dirs_string.replace(repeat, side + '+ ')
            
#        for side in SIDES: #if turns on one face followed by turns on other, switch them 
        #then call last for loop again
            
        for side in SIDES:#remove self-cancling turns
            cancel = side + '+ ' + side + '- '
            dirs_string.replace(cancel, "")
            cancel = side + '- ' + side + '+ '
            dirs_string.replace(cancel, "")
        
        self.directions = dirs_string.split()
...
def import_cube(interact = False):
    ...
    rubix_cube = list(map(lambda img: get_color_matrix(img, ref_dict = ref_dict), face_images))
...
rubix_key = gen_rubix_key(with_nums = False)
rubix_cube = import_cube(interact = False)
#print(rubix_cube)
